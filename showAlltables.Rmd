...
create_Proba <- function(indexWanted){
  ...
  if(!(is.na(up_flags[1, 1]))){
      ...
      if(k_up_flags['1', 'n'] > 4){
        ...
        # Determine the most possible longing target.
        if(k_up_flags['1', 'n'] > up_todo_PRO){
          up_todo_PRO <<- k_up_flags['1', 'n']
          up_todo_ind <<- indexWanted
        }
        ...
        returnSuggestion <- paste0(indexWanted, " is predicted to GO DOWN as\n", 
                                   ...
                                   round(ensemble_accuracy*100), "%.")
        # Determine the most possible shorting target.
        if(k_down_flags['1', 'n'] > dn_todo_PRO){
          dn_todo_PRO <<- k_down_flags['1', 'n']
          dn_todo_ind <<- indexWanted
        }
      }else{
...
MOST LONGING TARGET IS <font size="8" color="red"><b>`r up_todo_ind`</b></font> WITH PROBABILITY OF <font size="8" color="red"><b>`r up_todo_PRO`</b></font>.

MOST SHORTING TARGET IS <font size="8" color="green"><b>`r dn_todo_ind`</b></font> WITH PROBABILITY OF <font size="8" color="green"><b>`r dn_todo_PRO`</b></font>.
